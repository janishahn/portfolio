...
from app.services.github_client import get_client
...
def _write_summaries() -> None:
    ...
    try:
        SUMMARY_FILE.write_bytes(
            orjson.dumps(_readme_summary, option=orjson.OPT_INDENT_2)
        )
    except Exception as exc:
...
async def _generate_summary(
    readme_html: str, *, max_retries: int = 3, base_delay: float = 2.0
) -> str:
    ...
    model_slug = settings.openrouter_summary_model
    payload = {
        "model": model_slug,
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant that summarises README files.",
            },
    ...
    for attempt in range(1, max_retries + 1):
        try:
            logger.debug(
                "Making OpenRouter request (attempt %s/%s)", attempt, max_retries
            )
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                data = resp.json()
                logger.debug(
                    "OpenRouter request successful (status: %s)", resp.status_code
                )
                summary = (
        ...
        for name in missing:
            ...
            if not html:
                try:
                    ...
                    logger.debug(
                        "Failed to fetch README for %s during summary generation: %s",
                        name,
                        exc,
                    )
...
async def _generate_code_summary(
    code_text: str, *, max_retries: int = 3, base_delay: float = 2.0
) -> str:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant that summarises codebases.",
            },
            {"role": "user", "content": prompt},
        ],
        "temperature": 0.7,
    ...
    for attempt in range(1, max_retries + 1):
        try:
            logger.debug(
                "Making OpenRouter request for code summary (attempt %s/%s)",
                attempt,
                max_retries,
            )
            async with httpx.AsyncClient(timeout=60) as client:
            ...
            logger.warning(
                "Code summary generation attempt %s/%s failed: %s",
                attempt,
                max_retries,
                exc,
            )
